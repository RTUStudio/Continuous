...
public class QueueServer {
    ...
    private void createLimbo() {
        QueueConfig config = plugin.getQueueConfig();
        QueueConfig.World wc = config.world;
        QueueConfig.Location loc = wc.location();
        ...
        QueueConfig.Schematic schem = wc.schematic();
        if (schem.load()) {
            try {
                Path path = schematicPath.resolve(schem.file());
                WorldFile file = factory.openWorldFile(schem.type(), path);
                QueueConfig.Offset offset = schem.offset();
                file.toWorld(factory, world, offset.x(), offset.y(), offset.z(), wc.lightLevel());
                plugin.getLogger().info("Loaded queue schematic from {}", path);
            } catch (IOException e) {
                plugin.getLogger().warn("Failed to load queue schematic: {}", e.getMessage());
            }
        }
        
        this.limbo = factory.createLimbo(world)
                .setName("Continuous_Queue")
    ...
    public void send(Player player, RegisteredServer server) {
        ...
        QueueHandler handler = new QueueHandler(plugin, player, server);
        plugin.getHandlerManager().register(player, handler);
        limbo.spawnPlayer(player, handler);
        
        plugin.verbose("Sent " + player.getUsername() + " to queue for " + server.getServerInfo().getName());
    }
}
